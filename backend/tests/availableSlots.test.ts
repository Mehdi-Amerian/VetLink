...
describe('GET /api/availability/vets/:vetId/available-slots', () => {
  ...
  beforeAll(async () => {
    ...
    const vetUser = await prisma.user.create({ data: {
      email: 'vet@test.com', password: 'hashed', fullName: 'Dr Vet', role: 'VET', clinicId: clinic.id
    }});
    const vetProfile = await prisma.vet.create({ data: {
    ...
    await prisma.availability.createMany({ data: [
      { vetId, day: 'TUESDAY' as Weekday, startTime: '09:00', endTime: '12:00' },
      { vetId, day: 'TUESDAY' as Weekday, startTime: '13:00', endTime: '15:00' }
    ]});
